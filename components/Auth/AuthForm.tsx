...
import { createAccount, login } from "@/app/actions/auth";
import { useActionState } from "react";

export default function AuthTab() {
  const [state, action, isPending] = useActionState(createAccount, null);
  const [stateLogin, actionLogin] = useActionState(login, null);
  return (
        ...
        <TabsContent value="account">
          <form action={action} className="gap-2 flex flex-col">
            ...
            {state?.message && (
              <p
                className={`${
                  state.success ? "text-brand-hlg" : "text-red-500"
                } text-sm`}
              >
                {state.message}
              </p>
        ...
        <TabsContent value="password">
          <form action={actionLogin} className="gap-2 flex flex-col">
            ...
            {stateLogin?.message && (
              <p
                className={`${
                  stateLogin.success ? "text-brand-hlg" : "text-red-500"
                } text-sm`}
              >
                {stateLogin.message}
              </p>
